...
def convert_to_markdown_v2(output_data: dict,
    ...
    emojis_bitbucket = {
        "Can be split": ":twisted_rightwards_arrows:",
        "Key issues to review": ":zap:",
        "Recommended focus areas for review": ":zap:",
        "Score":":trophy:",
        "Relevant tests":":straight_ruler:",
        "Focused PR": ":sparkles:",
        "Relevant ticket": ":ticket:",
        "Security concerns": ":lock:",
        "Todo sections": ":memo:",
        "Insights from user's answers": ":memo:",
        "Code feedback": ":robot:",
        "Estimated effort to review [1-5]": ":stopwatch:",
        "Contribution time cost estimate": ":hourglass_flowing_sand:",
        "Ticket compliance check": ":ticket:",
    }
    ...
    markdown_text = ""
    use_shortcode_emojis = get_settings().get("bitbucket_server.use_shortcode_emojis", False)
    if use_shortcode_emojis == False:
        ...
        if not incremental_review:
            markdown_text += f"{PRReviewHeader.REGULAR.value} :mag:\n\n"
        else:
            markdown_text += f"{PRReviewHeader.INCREMENTAL.value} :mag:\n\n"
            markdown_text += f":rewind: Review for commits since previous PR-Agent review {incremental_review}.\n\n"
    ...
    for key, value in output_data['review'].items():
        ...
        if use_shortcode_emojis == False and gfm_supported == True:
            emoji = emojis.get(key_nice, "")
        else:
            emoji = emojis_bitbucket.get(key_nice, "")
        if 'Estimated effort to review' in key_nice:
            ...
            if use_shortcode_emojis == False and gfm_supported == True:
                ...
                blue_bars = ':large_blue_circle:' * value_int
                white_bars = ':white_circle:' * (5 - value_int)
            ...
            if all(level == 'Fully compliant' for level in all_compliance_levels):
                ...
                if any(level in ['Fully compliant', 'PR Code Verified'] for level in all_compliance_levels):
                    ...
                    if use_shortcode_emojis == False:
                        ...
                        compliance_emoji = ':large_orange_diamond:'
                else:
