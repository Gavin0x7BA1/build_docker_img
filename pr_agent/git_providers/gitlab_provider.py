...
class GitLabProvider(GitProvider):
    ...
    def get_diff_files(self) -> list[FilePatchInfo]:
        ...
        for diff in diffs:
            ...
            if counter_valid < MAX_FILES_ALLOWED_FULL or not diff['diff']:
                original_file_content_str = self.get_pr_file_content(diff['old_path'], self._get_merge_request_base_sha())
                new_file_content_str = self.get_pr_file_content(diff['new_path'], self._get_merge_request_head_sha())
            else:
    ...
    def get_languages(self):
        ver = get_settings().get("GITLAB.GITLAB_VERSION", None)
        if ver == "9.5.2": # gitlab support language since v10.x
            try:
                result = get_settings().get("GITLAB.GITLAB_LANG_DETECT", None)
                if result != None:
                    linguist_data = json.loads(result)
                    languages = {k: float(v["percentage"]) for k, v in linguist_data.items()}
                    get_logger().info(f"get_languags: {languages}")
                    return languages
                else:
                    get_logger().warning(f"Failed to get languages list from repo")
                    return None
            except:
                return None
        else:
    ...
    def _get_merge_request_base_sha(self):
        ver = get_settings().get("GITLAB.GITLAB_VERSION", None)
        if ver == "9.5.2": # gitlab support language since v10.x
            target_branch = self.mr.target_branch
            project = self.gl.projects.get(self.id_project)
            commits = project.commits.list(ref_name=target_branch, get_all=False)
            if commits:
                target_branch_commit = commits[0]
                #target_branch_commit = self.gl.projects.get(self.id_project).commits.list(ref_name=target_branch)[0]
                base_sha = target_branch_commit.id
        else:
            base_sha = self.mr.diff_refs['base_sha']
        return base_sha
    
    def _get_merge_request_head_sha(self):
        ver = get_settings().get("GITLAB.GITLAB_VERSION", None)
        if ver == "9.5.2": # gitlab support language since v10.x       
