...
def _create_custom_ssl_session(ssl_verify=True, ssl_ciphers=None,
                                ssl_security_level=None, ssl_legacy_support=False):
    """
    Creates a custom requests.Session with SSL/TLS configuration for legacy servers.

    Args:
        ssl_verify: bool or str, SSL verification (True/False or CA cert path)
        ssl_ciphers: str, custom cipher suite string
        ssl_security_level: int, OpenSSL security level (0-2)
        ssl_legacy_support: bool, enable legacy SSL support (sets SECLEVEL=1)
    ...
    if any([ssl_ciphers, ssl_security_level is not None, ssl_legacy_support]):
        # Create custom SSL context
        context = create_urllib3_context()

        # Set security level
        if ssl_legacy_support:
            context.set_ciphers('DEFAULT@SECLEVEL=1')
            get_logger().warning(
                "Legacy SSL support enabled - security reduced to SECLEVEL=1. "
                "This is NOT recommended for production use."
            )
        elif ssl_security_level is not None:
            context.set_ciphers(f'DEFAULT@SECLEVEL={ssl_security_level}')

        # Apply custom cipher suite if specified
        if ssl_ciphers:
            context.set_ciphers(ssl_ciphers)
            get_logger().info(f"Using custom SSL cipher suite: {ssl_ciphers}")

        # Create custom HTTPAdapter with the SSL context
    ...
    if not ssl_verify:
        get_logger().warning(
            "SSL verification is DISABLED for Bitbucket Server. "
            "This is NOT recommended for production use and exposes connections to security risks."
        )
...
class BitbucketServerProvider(GitProvider):
    def __init__(
        ...
        ssl_verify = get_settings().get("BITBUCKET_SERVER.SSL_VERIFY", True)
        ssl_ciphers = get_settings().get("BITBUCKET_SERVER.SSL_CIPHERS", None)
        ssl_security_level = get_settings().get("BITBUCKET_SERVER.SSL_SECURITY_LEVEL", None)
        ssl_legacy_support = get_settings().get("BITBUCKET_SERVER.SSL_LEGACY_SUPPORT", False)
        ...
        if bitbucket_client: # if Bitbucket client is provided, use it
            ...
            needs_custom_ssl = (
                ssl_verify is not True or
                ssl_ciphers or
                ssl_security_level is not None or
                ssl_legacy_support
            )
            ...
            if needs_custom_ssl:
                ...
                get_logger().info(
                    f"Creating custom SSL session for Bitbucket Server: "
                    f"verify={ssl_verify}, legacy_support={ssl_legacy_support}"
                )
                custom_session = _create_custom_ssl_session(
                    ssl_verify=ssl_verify,
                    ssl_ciphers=ssl_ciphers,
                    ssl_security_level=ssl_security_level,
                    ssl_legacy_support=ssl_legacy_support
                )

                # Initialize Bitbucket client with custom session
                ...
                # Use default initialization (backward compatible)
